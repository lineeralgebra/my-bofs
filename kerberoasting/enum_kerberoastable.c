...
void go(char* args, int len) {
    ...
    PWSTR attrs[] = { L"sAMAccountName", L"servicePrincipalName", NULL };

    res = WLDAP32$ldap_search_ext_sW(ld, baseDN, LDAP_SCOPE_SUBTREE, L"(&(objectClass=user)(!(objectClass=computer))(servicePrincipalName=*)(!(sAMAccountName=krbtgt)))", attrs, 0, NULL, NULL, NULL, 0, &searchResult);
    ...
    while (entry != NULL) {
        ...
        PWSTR* spn_vals = WLDAP32$ldap_get_valuesW(ld, entry, L"servicePrincipalName");

        if (user_vals != NULL) {
            BeaconFormatPrintf(&buffer, "%-20ls | %-30ls\n", user_vals[0], (spn_vals != NULL) ? spn_vals[0] : L"");
        }

        if (user_vals) WLDAP32$ldap_value_freeW(user_vals);
        if (spn_vals) WLDAP32$ldap_value_freeW(spn_vals);
