...
void go(char* args, int len) {
    ...
    PWSTR attrs[] = { L"sAMAccountName", L"dNSHostName", L"operatingSystem", NULL };

    res = WLDAP32$ldap_search_ext_sW(ld, baseDN, LDAP_SCOPE_SUBTREE, L"(objectClass=computer)", attrs, 0, NULL, NULL, NULL, 0, &searchResult);
    ...
    /* Use formatp to buffer all output into a single chunk */
    formatp buffer;
    BeaconFormatAlloc(&buffer, 16 * 1024);

    BeaconFormatPrintf(&buffer, "[+] Using Base DN: %ls\n\n", baseDN ? baseDN : L"N/A");

    DWORD count = 0;
    LDAPMessage* entry = WLDAP32$ldap_first_entry(ld, searchResult);
    while (entry != NULL) {
        PWSTR* name = WLDAP32$ldap_get_valuesW(ld, entry, L"sAMAccountName");
        PWSTR* dns = WLDAP32$ldap_get_valuesW(ld, entry, L"dNSHostName");
        PWSTR* os = WLDAP32$ldap_get_valuesW(ld, entry, L"operatingSystem");

        BeaconFormatPrintf(&buffer, "Computer: %ls (DNS: %ls, OS: %ls)\n",
            name ? name[0] : L"N/A",
            dns ? dns[0] : L"N/A",
            os ? os[0] : L"N/A");

        if (name) WLDAP32$ldap_value_freeW(name);
        if (dns) WLDAP32$ldap_value_freeW(dns);
        if (os) WLDAP32$ldap_value_freeW(os);

        entry = WLDAP32$ldap_next_entry(ld, entry);
        count++;
    }

    BeaconFormatPrintf(&buffer, "\nTotal computers found: %d\n", count);

    /* Send everything as one output block */
    int outLen = 0;
    char* outData = BeaconFormatToString(&buffer, &outLen);
    BeaconOutput(CALLBACK_OUTPUT, outData, outLen);
